...
ptr_velo_raw = (centroid(2:end,:) - centroid(1:end-1,:)) / (dt * px_mm);  % Particle velocity (mm/s)
ptr_velo_centroid = (filtered_centroid(2:end,:) - filtered_centroid(1:end-1,:)) / (dt * px_mm);  % Particle velocity (mm/s)

% due to over fitting of polynomial filter, I used raw velocity data at
% first and last two points (2 = floor(5/2));
ptr_velo_fin = ptr_velo_centroid;
ptr_velo_fin(1:2,:) = ptr_velo_raw(1:2,:);
ptr_velo_fin(num_img-1:num_img,:) = ptr_velo_raw(num_img-1:num_img,:);
...
plot(tt_dom, ptr_velo_fin(:,1)/10, '-o');
hold on;
...
plot(tt_dom, ptr_velo_fin(:,2)/10, '-o');
hold on;
...
h_fig(1) = subplot(2,1,1);
plot(tt_dom, (ptr_velo_fin(:,1) - ptr_velo_fluid(:,1)) / 10, '-*');
hold on;
...
h_fig(2) = subplot(2,1,2);
plot(tt_dom, (ptr_velo_fin(:,2) + ptr_velo_fluid(:,2)) / 10, '-x');
hold on;
